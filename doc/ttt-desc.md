...
- **Backend**
  - Written in Java, using Vert.x event-loop-based framework. It exposes an HTTP endpoint providing a REST-style API to register users, create new games, join games, and play moves.
    ...
    - To run it from the command line:
      ...
      If port `8080` is already in use, pass a different port:

      `mvn compile exec:java -Dexec.mainClass="ttt_backend.TTTBackend" -Dexec.args="8081"`
  
  - Notes about the design and implementation: 
    - `ttt_backend.TTTBackend` is a big monolitic backend implemented using Vert.x + Web reactive framework. The backend receives HTTP requests (port `8080`) and processes them. The REST-style API is:
      - `POST /api/users` registers a new user to the game server, given its user name. 
        - It generates a unique user id. An hash map `users` is used to keep track of the set of registered users (class (`domain.model.User`). 
        - A simple JSON dbase (`users.json` file) is used to persist the set of registered users.  
      - `POST /api/games` creates a new game. Each game has its own game id and it is represented by the class `domain.model.Game`. An hash map `games` is used to keep track of the ongoing games.
      ...
      - `PUT /api/games/{gameId}/players/{userId}` joins an existing game, given the symbol to be used (cross or circle) in the request body.
        - When a user joins a game, a websocket can be created at `/api/games/{gameId}/events` to notify game events to the frontend. 
      ...
      - `POST /api/games/{gameId}/moves` makes a move in a game, specifying who wants to move (circle or cross) and where to move (x and y coordinates of the game grid, from 0 to 2)
